        ...
        if (data.Activated) {
            ...
            */
            if (data.AccumulatedDamageMultiplier >= DamageInterval) {
                data.Target.TakeDamage (DamagePerSecond * data.AccumulatedDamageMultiplier * HeatDamageMultiplier.Evaluate (data.Heat / MaxHeat), slot.Equipper.transform.position);
