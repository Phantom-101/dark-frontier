...
namespace DarkFrontier.Foundation.Services {
    public static class Singletons {
        ...
        public static T Get<T> (string key = "") where T : class {
            try {
                return (Instances.TryGet (typeof (T), new Dictionary<string, object> ())[key] as T)!;
            } catch (Exception) {
                throw new Exception ($"Service of type {typeof (T)} not found");
            }
        }

        public static bool Bind<T> (T instance, string key = "") where T : class {
            try {
                Instances.TryAdd (typeof (T), new Dictionary<string, object> ());
                Instances[typeof (T)].Add (key, instance!);
                return true;
            } catch (Exception) {
                return false;
            }
