...
public class Faction : ISaveTo<FactionSaveData> {
    ...
    public void SetRelation (string factionId, float target) { if (factionId != null) { relations[factionId] = Mathf.Clamp (target, -1, 1); factionManager.Registry.Find (factionId).SetRelationBack (Id, target); } }
