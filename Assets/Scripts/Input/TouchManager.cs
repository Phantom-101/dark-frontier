...
public class TouchManager : SingletonBase<TouchManager> {
    ...
    private void Update () {
        ...
        // Tick delta time
        deltaTimeSinceLastMouseMove += Time.deltaTime;
    ...
    private void GetMouseDeltaPosition () {
        // Get mouse delta position
        mouseDeltaPosition = new Vector2 (Input.mousePosition.x, Input.mousePosition.y) - lastFrameMousePosition;
    ...
    private void RecordRawMousePositions () {
        // Left click
        if (Input.GetMouseButtonDown (0)) leftClickRawMousePosition = Input.mousePosition;
        // Right click
        if (Input.GetMouseButtonDown (1)) rightClickRawMousePosition = Input.mousePosition;
        // Middle click
        if (Input.GetMouseButtonDown (2)) middleClickRawMousePosition = Input.mousePosition;
    }

    private void SetTouchPhases () {
        // Left click
        if (Input.GetMouseButtonDown (0)) leftClickTouchPhase = TouchPhase.Began;
        else if (Input.GetMouseButtonUp (0)) leftClickTouchPhase = TouchPhase.Ended;
        else if (Input.GetMouseButton (0)) leftClickTouchPhase = didMouseMoveSinceLastFrame ? TouchPhase.Moved : TouchPhase.Stationary;
        // Right click
        if (Input.GetMouseButtonDown (1)) rightClickTouchPhase = TouchPhase.Began;
        else if (Input.GetMouseButtonUp (1)) rightClickTouchPhase = TouchPhase.Ended;
        else if (Input.GetMouseButton (1)) rightClickTouchPhase = didMouseMoveSinceLastFrame ? TouchPhase.Moved : TouchPhase.Stationary;
        // Middle click
        if (Input.GetMouseButtonDown (2)) middleClickTouchPhase = TouchPhase.Began;
        else if (Input.GetMouseButtonUp (2)) middleClickTouchPhase = TouchPhase.Ended;
        else if (Input.GetMouseButton (2)) middleClickTouchPhase = didMouseMoveSinceLastFrame ? TouchPhase.Moved : TouchPhase.Stationary;
    }

    private List<Touch> GetTouches () {
        // Get finger touches
        List<Touch> ret = Input.touches.ToList ();

        // Convert mouse clicks to touches

        // Left click
        if (Input.GetMouseButton (0)) ret.Add (GetLeftClickTouch ());
        // Right click
        if (Input.GetMouseButton (1)) ret.Add (GetRightClickTouch ());
        // Middle click
        if (Input.GetMouseButton (2)) ret.Add (GetMiddleClickTouch ());
